...
dat.excess <- data_mortality_rate_sex %>%
  ...
  select(Year,death, population,Region, Region.struct,sex)
...
formula <- death ~ 1 + offset(log(population)) +
  ...
  write.xlsx(expected_deaths,paste0("data/expected_death_inla",Sex,"_",Year_Pan,".xlsx"), row.names=FALSE, overwrite = TRUE)
  save(expected_deaths,file=paste0("data/expected_death_inla",Sex,"_",Year_Pan,".RData"))
  }

function_inla_sex(Year_Pan=1890, Year_max=1895, Year_min=1882, Sex="f")
function_inla_sex(Year_Pan=1890, Year_max=1895, Year_min=1882, Sex="m")

function_inla_sex(Year_Pan=1918, Year_max=1920, Year_min=1908, Sex="f")
function_inla_sex(Year_Pan=1918, Year_max=1920, Year_min=1908, Sex="m")

function_inla_sex(Year_Pan=2020, Year_max=2020, Year_min=2014, Sex="f")
function_inla_sex(Year_Pan=2020, Year_max=2020, Year_min=2014, Sex="m")
