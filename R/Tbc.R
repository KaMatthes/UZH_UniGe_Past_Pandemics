...
Tbc1890 <- readxl::read_excel("data_raw/Tubercluosis_deaths.xlsx", sheet="1890") %>%
  filter(!Scale =="Canton") %>%
  select(Year,Canton=CantonShort,MapName, Tbc = `Deaths TB 1886_1890`)
  
Tbc1918 <- readxl::read_excel("data_raw/Tubercluosis_deaths.xlsx", sheet="1918") %>%
  filter(!Scale =="Canton") %>%
  select(Year,Canton=CantonShort,MapName, Tbc = `Tb Deaths 1911_1920`) 



Tbc_data <- rbind(Tbc1890, Tbc1918) %>%
...
pop_total1886_1890 <- pop_total %>%
  filter(between(Year, 1886,1890)) %>%
  group_by(Bezirk) %>%
  mutate(pop_sum= sum(population))%>%
  ungroup() %>%
  distinct(Bezirk,.keep_all = TRUE)

pop_total1911_1920 <- pop_total %>%
  filter(between(Year, 1911,1920)) %>%
  group_by(Bezirk) %>%
  mutate(pop_sum= sum(population))%>%
  ungroup() %>%
  distinct(Bezirk,.keep_all = TRUE) %>%
  rbind(pop_total1886_1890) %>%
  select(Year, Bezirk, pop_sum) %>%
...
Tbc <- Tbc_data %>%
  ...
  left_join(pop_total1911_1920)
  # mutate(Inc = Tbc_Bezirk/pop_sum *10000)

write.xlsx(Tbc ,file=paste0("data/Tbc.xlsx"),row.names=FALSE, overwrite = TRUE)
# save
save(Tbc ,file=paste0("data/Tbc.RData"))