...
Urbanity <- readxl::read_excel("data_raw/Hospitals.xlsx") %>%
  filter(year == 1915) %>%
  select(MapName, city) %>%
  dplyr::mutate( 
  ...
  dplyr::group_by(Bezirk,MapName) %>%
  mutate(city_bezirk = mean(city)) %>%
  ungroup() %>%
  distinct(Bezirk , .keep_all = TRUE) %>%
  mutate(city_bezirk = ifelse(city_bezirk>0 & city_bezirk <1, 1,city_bezirk)) %>%
  select(-city)

write.xlsx(Urbanity,file=paste0("data/Urbanity.xlsx"),row.names=FALSE, overwrite = TRUE)
save(Urbanity,file=paste0("data/Urbanity.RData"))
