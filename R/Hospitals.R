...
Hospitals <- readxl::read_excel("data_raw/Hospitals.xlsx") %>%
  filter(year==1890 | year == 1915) %>%
  dplyr::mutate( 
                MapName=recode(MapName, "de Courtelary"  = "Jura bernois",
                               ...
                               "Niedersimmenthal" = "Frutigen-Niedersimmental",
                               "Obersimmenthal" = "Obersimmental-Saanen",
                               "Gaster" = "See-Gaster",
                               "Glenner"	= "Surselva",
                               "Gossau" = "St. Gallen",
                               "Gallen" = "St. Gallen",
                               ...
                               "Baselstadt Landbezirk"= "Basel-Stadt",
                               "Baselstadt Stadtbezirk"= "Basel-Stadt",
                               "Lauis"	= "Lugano",
                               ...
                               "Klettgau, Unter-" = "Kanton Schaffhausen",
  ...
  select(Bezirk,MapName,year, hospitals_total) %>%
  dplyr::group_by(Bezirk,MapName,year) %>%
summarise(hospitals = sum(hospitals_total)) %>%
  ungroup() %>%
  distinct(Bezirk, year, hospitals , .keep_all = TRUE) %>%
  rename(Year=year)

write.xlsx(Hospitals,file=paste0("data/Hospitals.xlsx"),row.names=FALSE, overwrite = TRUE)
save(Hospitals,file=paste0("data/Hospitals.RData"))
