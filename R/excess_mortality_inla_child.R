function_inla_child <- function(Year_Pan,Year_max, Year_min) {
  load(paste0("data/data_mortality_rate_child.RData"))
...
dat.excess <- data_mortality_rate_child %>%
...
formula <- death ~ 1 + offset(log(population)) +
  ...
  write.xlsx(expected_deaths,paste0("data/expected_death_inla_child",Year_Pan,".xlsx"), row.names=FALSE, overwrite = TRUE)
  save(expected_deaths,file=paste0("data/expected_death_inla_child",Year_Pan,".RData"))
  }

function_inla_child(Year_Pan=1890, Year_max=1895, Year_min=1882)
function_inla_child(Year_Pan=1918, Year_max=1920, Year_min=1908)
