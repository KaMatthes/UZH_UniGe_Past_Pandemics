...
1918
Age groups 0-19, 20-39 and 40-69 show higher excess mortality compared to >70 years. The  highest excess mortality shows the age group 0-69.
...
function_cor_tbc(Davos="yes")
```

Without Davos

```{r, echo=FALSE,message=FALSE,warning=FALSE,fig.height = 10,  fig.width = 12}
function_cor_tbc(Davos="no")
```
...
However, excess mortality is lower in all 3 pandemics the higher the population density. The reason for this could perhaps be the better medical care in larger cities? The greater proximity to the hospitals or medical office?

```{r, echo=FALSE,message=FALSE,warning=FALSE,fig.height = 10,  fig.width = 12}
function_cor_area(city="yes")
```

Without the districts: City of Zurich, Basel-Stadt, Lausanne, Geneva

```{r, echo=FALSE,message=FALSE,warning=FALSE,fig.height = 10,  fig.width = 12}
function_cor_area(city="no")
```

Regression analysis with all districts. Population density is not significantly associated with excess mortality.
